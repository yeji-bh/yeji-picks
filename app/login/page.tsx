...
export default function LoginPage() {
  ...
  return (
    <div className="mx-auto w-full max-w-sm min-w-0">
      <h1 className="mb-2 text-xl font-semibold">{t("auth.login")}</h1>
      <p className="mb-6 text-sm text-muted">{t("auth.loginDesc")}</p>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <p className="text-center text-sm text-muted">
          {t("auth.noAccount")}{" "}
          <Link href="/register" className="text-neutral-900 underline hover:text-neutral-600">
            {t("auth.register")}
          </Link>
        </p>
      </form>
